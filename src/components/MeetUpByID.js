import PageTransition from "./PageTransition";

export default function MeetUpByID() {
  return (
    <PageTransition>
      <section className="pt-[120px] pb-[80px] lg:pt-[100px] lg:pb-[30px] bg-blue relative overflow-hidden">
        <div className="container mx-auto px-10">
          <div className="meetup-heading">
            <h1>Meetup Title</h1>
            <div className="grid gap-9 lg:grid-cols-12 grid-cols-1 items-center">
              <div className="lg:col-span-1">
                <img
                  className="avatar mt-3"
                  src="data:image/jpeg;base64,"
                ></img>
              </div>
              <div className="lg:col-span-4">
                <ul>
                  <li>Hosted by</li>
                  <li>
                    <strong>Mai</strong>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section className="pt-[120px] pb-[80px] lg:pt-[100px] lg:pb-[30px] relative overflow-hidden">
        <div className="container mx-auto px-10">
          <div className="grid lg:grid-cols-12 grid-cols-1 items-center">
            <div className="lg:col-span-8 text-center mb-[50px] lg:mb-0"></div>
            <div className="lg:col-span-4 text-center mb-[50px] lg:mb-0"></div>
          </div>
        </div>
      </section>
    </PageTransition>
  );
}
